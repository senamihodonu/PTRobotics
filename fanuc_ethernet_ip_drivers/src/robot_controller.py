...
class robot:
    def __init__(self, robotIP: str, DEBUG: bool=False):
        ...
        self.sync_register = 2 # gripper/conveyor?
        self.sync_value = 1 
        ...
        print("Connection made with robot at ", self.robot_IP)
    ...
    def read_joint_position_register(self) -> float:
        ...
        """
        PR_1_Value = FANUCethernetipDriver.readJointPositionRegister(self.robot_IP, self.PRNumber1)
        #print("PR[%d]"% self.PRNumber1)
    ...
    # Takes x, y, z, w, p, r coords.
    # WPR are the orientation of the end effector, DEFAULT to current orientation
    def write_cartesian_position_2(self, coords:list[float], blocking:bool=True):
        """! Send cartesian coordinates to robot using X, Y, Z, W, P, R system. 
        These coordinates usually correlate to the tool end effectors position.
        @param coords[X, Y, Z, W, P, R]  OR  coords[[X,Y,Z,W,P,R], [X,Y,Z,...], ...]
            - X cartesian coordinate
    ...
    # Takes x, y, z, w, p, r coords.
    # WPR are the orientation of the end effector, DEFAULT to current orientation
    def write_cartesian_position_3(self, coords:list[float], blocking:bool=True):
        """! Send cartesian coordinates to robot using X, Y, Z, W, P, R system. 
        These coordinates usually correlate to the tool end effectors position.
        @param coords[X, Y, Z, W, P, R]  OR  coords[[X,Y,Z,W,P,R], [X,Y,Z,...], ...]
            - X cartesian coordinate
    ...
    def get_speed(self) -> int:
        ...
        # Utility Functions
    # write R[5] to set Speed in mm/sec
    def set_robot_speed_percent(self, value: int):
        """! Set movement speed of robot in mm/s
        @param value        speed in mm/s
        """
        print("------------------------------")
        print(f"| Speed percent set to {value}% |")
        print("------------------------------")
    ...
    def get_robot_speed_percent(self) -> int:
        """
        Returns the current commanded speed percentage of the robot.

        Returns:
            int: Speed as a percentage (0–100).
        """
        return FANUCethernetipDriver.readR_Register(self.robot_IP, self.speed_percent)
    
    def get_actual_robot_speed(self) -> float:
        """
        Calculates the actual robot speed in mm/sec based on the set speed and the current speed percentage.

        Returns:
            float: Actual speed in mm/sec.
        """
        base_speed = self.get_speed()  # Maximum or nominal speed
        percent = self.get_robot_speed_percent()
        actual_speed = (percent / 100.0) * base_speed
        return actual_speed
